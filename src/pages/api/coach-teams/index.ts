...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (req.method) {
    case 'GET':
      return getCoachTeams();
    case 'POST':
      return createCoachTeam();
    default:
      return res.status(405).json({ message: `Method ${req.method} not allowed` });
  }

  async function getCoachTeams() {
    const data = await prisma.coach_team
      .withAuthorization({
        roqUserId,
        tenantId: user.tenantId,
        roles: user.roles,
      })
      .findMany(convertQueryToPrismaUtil(req.query, 'coach_team'));
  ...
  async function createCoachTeam() {
    ...
    const data = await prisma.coach_team.create({
      data: body,
    });
    return res.status(200).json(data);
  }
