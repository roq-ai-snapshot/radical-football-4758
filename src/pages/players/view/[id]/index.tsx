...
import { deleteParentChildById } from 'apiSdk/parent-children';
...
function PlayerViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<PlayerInterface>(
    ...
    () =>
      getPlayerById(id, {
        relations: ['user', 'team', 'attendance', 'parent_child', 'player_training_program'],
      }),
  );
  ...
  const parent_childHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
          ...
          <>
            ...
            {hasAccess('parent_child', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
              <>
                ...
                <NextLink passHref href={`/parent-children/create?child_id=${data?.id}`}>
                  <Button colorScheme="blue" mr="4" as="a">
                    Create
                  </Button>
                </NextLink>
                <TableContainer>
                  <Table variant="simple">
                    ...
                    <Tbody>
                      {data?.parent_child?.map((record) => (
                        <Tr key={record.id}>
                          <Td>
                            <NextLink passHref href={`/parent-children/edit/${record.id}`}>
                              <Button as="a">Edit</Button>
                            </NextLink>
                          </Td>
                          <Td>
                            <NextLink passHref href={`/parent-children/view/${record.id}`}>
                              <Button as="a">View</Button>
                            </NextLink>
                          </Td>
                          <Td>
                            <Button onClick={() => parent_childHandleDelete(record.id)}>Delete</Button>
                          </Td>
                        </Tr>
                      ))}
                    </Tbody>
            ...
            {hasAccess('player_training_program', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
              <>
                ...
                <NextLink passHref href={`/player-training-programs/create?player_id=${data?.id}`}>
                  <Button colorScheme="blue" mr="4" as="a">
                    Create
                  </Button>
                </NextLink>
